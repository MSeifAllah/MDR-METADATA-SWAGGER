...
function metadata(model, nameSpace) {
    return buildMetadata(model, nameSpace);
}
exports.metadata = metadata;
function buildMetadata(models, nameSpace) {
    let entitesType = [];
...
function getEntitiesType(model, nameSpace) {
    let compositionType = [];
    ...
    for (let prop in props) {
        if (prop == model._autoIncrementId)
            continue; // traitemant des clé à la fin
...
function getEntitiesContainer(model, nameSpace, container) {
...
function getOdataType(prop, nameSpace) {
    ...
    switch (prop.type) {
        ...
        case "array":
            if (prop.items)
                return "Collection(" + getOdataType(prop.items, nameSpace) + ")";
